...
class SquaredLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        curr_index = 0
        for x in X:
            terms.append(np.dot(w.T, x))
            curr_index+=1
        
        difference = y - np.array(terms)
        first_term = 1/(2*X.shape[0])
        second_term = np.sum(difference**2)
    ...
    def backward(self, X, w, y):
        ...
        Returns:
            gradient - (np.array) The (d+1)-dimensional gradient of the loss
                ...
                the bias term.
        """
        second_term = np.zeros(X.shape[0])
        curr_index = 0
        for x in X:
            second_term[curr_index] = y[curr_index] - np.dot(w.T,x)
            curr_index+=1
        
        gradient = -1 * np.dot(X.T, second_term) / (y.shape[0])
...
class HingeLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        term = []
        for x in X:
            term.append(max(0, 1 - y[curr_index] * np.dot(w.T,x)))
            curr_index+=1
    ...
    def backward(self, X, w, y):
        ...
        for x in X:
            term =  1 - (y[curr_index] * np.dot(w.T,x))
            if term >= 0:
                gradient +=  -x * y[curr_index] / N
            curr_index+=1
